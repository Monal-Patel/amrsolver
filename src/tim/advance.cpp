...
// Since we do not want to use expensive cudaMemCopy, we are storing all our data on the GPU to begin with. Concurrency on GPU using streams, parallel computation and data transfer, is not useful then. Therefore, we can have all grid point computations, per fab, in a single MFIter loop (single stream).

// However, computation and temporary space allocation could be done concurrently?
                   ...
                   FluxRegister* fr_as_crse, FluxRegister* fr_as_fine)
{
  ...
  for (MFIter mfi(consmf, false); mfi.isValid(); ++mfi) {
    ...
    // We want to minimise function calls. So, we call prims2cons, flux and source term evaluations once per fab from CPU, to be run on GPU.
  ...
  //     MultiFab lambdamf; lambdamf.define(consmf.boxArray(), consmf.DistributionMap(), AMREX_SPACEDIM, NGHOST);
  //     MultiFab sensormf; sensormf.define(consmf.boxArray(), consmf.DistributionMap(), AMREX_SPACEDIM, NGHOST);
  //     lambdamf = 0.0_rt;
  //     sensormf = 0.0_rt;
...
//               ViscousWallFluxes(i, j, k, 0, primsfab, pfabfx, pfabfy, pfabfz, dxinv, cls);
//           });
//         }
//       }
//       //yhi
...
//           amrex::ParallelFor(bxboundary,[=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept {
//               ViscousWallFluxes(i, j, k, 1, primsfab, pfabfx, pfabfy, pfabfz, dxinv, cls);
//           });
//         }
//       }
//       //zlo
