...
Real cns_estdt (Box const& bx, Array4<Real const>   const& state, GpuArray<Real,AMREX_SPACEDIM> const& dx, const PROB::ProbClosures& closures) noexcept
{
    ...
    for         (int k = lo.z; k <= hi.z; ++k) {
        for     (int j = lo.y; j <= hi.y; ++j) {
            for (int i = lo.x; i <= hi.x; ++i) {
                ...
                Real cs = std::sqrt(closures.gamma*p*rhoinv);
...
AMREX_GPU_DEVICE AMREX_FORCE_INLINE void pointCFL (int i, int j, int k, Array2D<Real,0,2,0,2>& array, const Array4<const Real>& prims, const  PROB::ProbClosures& closures,const auto dx, Real dt) noexcept
{
  Real sos = std::sqrt(closures.gamma*closures.Rspec*prims(i,j,k,QT));
  //
  for (int idir=0; idir<AMREX_SPACEDIM; idir++){
    ...
    // Print() << "inside " << array(idir,0) << " " << array(idir,1) << " " << array(idir,2) << std::endl; 
  }
}


AMREX_GPU_DEVICE AMREX_FORCE_INLINE
void cns_compute_temperature (int i, int j, int k, Array4<Real> const& u,
                         const PROB::ProbClosures& closures) noexcept
{
