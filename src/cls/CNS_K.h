...
AMREX_GPU_DEVICE AMREX_FORCE_INLINE void pointCFL(
    ...
    const Array4<const Real>& prims, const PROB::ProbClosures& closures,
    const auto dx, Real dt) noexcept {
  Real sos = std::sqrt(closures.gamma * closures.Rspec * prims(i, j, k, QT));
  //
  for (int idir = 0; idir < AMREX_SPACEDIM; idir++) {
    array(idir, 0) =
        max(array(idir, 0), std::abs(prims(i, j, k, QU + idir) +
                                     sos));  // max(|ui + c|) in +ith direction
    array(idir, 1) =
        max(array(idir, 1), std::abs(prims(i, j, k, QU + idir) -
                                     sos));  // max(|ui - c|) in -ith direction
...
AMREX_GPU_DEVICE AMREX_FORCE_INLINE void cns_compute_temperature(
  ...
  Real mz = u(i, j, k, UMZ);
  Real rhoeint =
      u(i, j, k, UET) - Real(0.5) * rhoinv * (mx * mx + my * my + mz * mz);
...
inline void derpres(const Box& bx, FArrayBox& derfab, int dcomp, int /*ncomp*/,
  ...
  amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
    ...
    Real rhoeint =
        dat(i, j, k, UET) - Real(0.5) * rhoinv * (mx * mx + my * my + mz * mz);
