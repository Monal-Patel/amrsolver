...
AMREX_GPU_DEVICE AMREX_FORCE_INLINE void cons2eulerflux(
    int i, int j, int k, auto const& cons, auto const& fx, auto const& fy,
    auto const& fz, const PROB::ProbClosures& closures) {
...
// void cons2char (int i, int j, int k, int dir, Array4<const Real> const& q,
// const Array4<const Real>& w, const int sys)
// {
//     int QUN, QUT, QUTT;
...
//         // Real rmpoCshock = q(i,j,k,QRHO) -
//         q(i,j,k,QPRES)/q(i,j,k,QC)/q(i,j,k,QC); //rho minus p over c^2
//         w(i,j,k,WRHO) = q(i,j,k,QRHO) -
//         q(i,j,k,QPRES)/q(i,j,k,QC)/q(i,j,k,QC); //rho minus p over c^2
//         w(i,j,k,WACO)   = 0.5 * (q(i,j,k,QPRES)/q(i,j,k,QC) +
//         q(i,j,k,QRHO)*q(i,j,k,QUN)); w(i,j,k,WACO+1) = 0.5 *
//         (q(i,j,k,QPRES)/q(i,j,k,QC) - q(i,j,k,QRHO)*q(i,j,k,QUN));

//     } else { // gamma system
//         w(i,j,k,WRHO) = q(i,j,k,QRHO) * (1.0 - 1.0 / q(i,j,k,QG));
//         w(i,j,k,WACO) = 0.5 * (q(i,j,k,QPRES) +
//         std::sqrt(q(i,j,k,QG)*q(i,j,k,QRHO)*q(i,j,k,QPRES))*q(i,j,k,QUN));
//         w(i,j,k,WACO+1) = 0.5 * (q(i,j,k,QPRES) -
//         std::sqrt(q(i,j,k,QG)*q(i,j,k,QRHO)*q(i,j,k,QPRES))*q(i,j,k,QUN));
//     }
...
AMREX_GPU_DEVICE AMREX_FORCE_INLINE void cons2eulerflux_lambda(
    int i, int j, int k, auto const& cons, auto const& fx, auto const& fy,
    auto const& fz, auto const& lambda, const PROB::ProbClosures& closures) {
  Real rhoinv = Real(1.0) / cons(i, j, k, URHO);
  Real momx = cons(i, j, k, UMX);
  Real momy = cons(i, j, k, UMY);
  Real momz = cons(i, j, k, UMZ);
...
AMREX_GPU_DEVICE AMREX_FORCE_INLINE void numericalflux(
...
AMREX_FORCE_INLINE void FluxWENO(MultiFab& statemf, MultiFab& primsmf,
                                 Array<MultiFab, AMREX_SPACEDIM>& numflxmf) {
  // make multifab for variables
  Array<MultiFab, AMREX_SPACEDIM> pntflxmf;
  for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
    pntflxmf[idim].define(statemf.boxArray(), statemf.DistributionMap(), NCONS,
                          NGHOST);
  }

  // store eigenvalues max(u+c,u,u-c) in all directions
  MultiFab lambdamf;
  lambdamf.define(statemf.boxArray(), statemf.DistributionMap(), AMREX_SPACEDIM,
                  NGHOST);
  ...
  for (MFIter mfi(statemf, false); mfi.isValid(); ++mfi) {
    ...
    const Box& bxnodal =
        mfi.nodaltilebox();  // extent is 0,N_cell+1 in all directions -- -1
                             // means for all directions.
                             // amrex::surroundingNodes(bx) does the same
    ...
    ParallelFor(bxg, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
      cons2eulerflux_lambda(i, j, k, statefab, pfabfx, pfabfy, pfabfz, lambda,
                            lclosures);
    });
    ...
    ParallelFor(bxnodal, int(NCONS),
                [=] AMREX_GPU_DEVICE(int i, int j, int k, int n) noexcept {
                  numericalflux_globallaxsplit(i, j, k, n, statefab, pfabfx,
                                               pfabfy, pfabfz, lambda, nfabfx,
                                               nfabfy, nfabfz);
                });
  }
}
