...
// void cons2prim (const Box& bxg, const Array4<const Real>& u, Array4<Real>
// const& q, const PROB::ProbClosures& closures) noexcept
// {
...
AMREX_GPU_DEVICE AMREX_FORCE_INLINE void ViscousFluxes(
  ...
  Real ux = prims(i, j, k, QU);
  Real dudx =
      (prims(i + 1, j, k, QU) - prims(i - 1, j, k, QU)) * 0.5_rt * dxinv[0];
  Real dvdx =
      (prims(i + 1, j, k, QV) - prims(i - 1, j, k, QV)) * 0.5_rt * dxinv[0];
  Real dwdx =
      (prims(i + 1, j, k, QW) - prims(i - 1, j, k, QW)) * 0.5_rt * dxinv[0];
  Real dTdx =
      (prims(i + 1, j, k, QT) - prims(i - 1, j, k, QT)) * 0.5_rt * dxinv[0];

  // y direction
  Real uy = prims(i, j, k, QV);
  Real dudy =
      (prims(i, j + 1, k, QU) - prims(i, j - 1, k, QU)) * 0.5_rt * dxinv[1];
  Real dvdy =
      (prims(i, j + 1, k, QV) - prims(i, j - 1, k, QV)) * 0.5_rt * dxinv[1];
  Real dwdy =
      (prims(i, j + 1, k, QW) - prims(i, j - 1, k, QW)) * 0.5_rt * dxinv[1];
  Real dTdy =
      (prims(i, j + 1, k, QT) - prims(i, j - 1, k, QT)) * 0.5_rt * dxinv[1];

  // z direction
  Real uz = prims(i, j, k, QW);
  Real dudz =
      (prims(i, j, k + 1, QU) - prims(i, j, k - 1, QU)) * 0.5_rt * dxinv[2];
  Real dvdz =
      (prims(i, j, k + 1, QV) - prims(i, j, k - 1, QV)) * 0.5_rt * dxinv[2];
  Real dwdz =
      (prims(i, j, k + 1, QW) - prims(i, j, k - 1, QW)) * 0.5_rt * dxinv[2];
  Real dTdz =
      (prims(i, j, k + 1, QT) - prims(i, j, k - 1, QT)) * 0.5_rt * dxinv[2];
...
AMREX_GPU_DEVICE AMREX_FORCE_INLINE void ViscousWallFluxes(
  ...
  // bottom boundary
  if (loc == 0) {
    dudy = (-prims(i, j + 2, k, QU) + 4 * prims(i, j + 1, k, QU) -
            3 * prims(i, j, k, QU)) *
           0.5_rt * dxinv[1];
    dvdy = (-prims(i, j + 2, k, QV) + 4 * prims(i, j + 1, k, QV) -
            3 * prims(i, j, k, QV)) *
           0.5_rt * dxinv[1];
    dwdy = (-prims(i, j + 2, k, QW) + 4 * prims(i, j + 1, k, QW) -
            3 * prims(i, j, k, QW)) *
           0.5_rt * dxinv[1];
    dTdy = (-prims(i, j + 2, k, QT) + 4 * prims(i, j + 1, k, QT) -
            3 * prims(i, j, k, QT)) *
           0.5_rt * dxinv[1];
  }
  // top boundary
  else if (loc == 1) {
    dudy = (3 * prims(i, j, k, QU) - 4 * prims(i, j - 1, k, QU) +
            prims(i, j - 2, k, QU)) *
           0.5_rt * dxinv[1];
    dvdy = (3 * prims(i, j, k, QV) - 4 * prims(i, j - 1, k, QV) +
            prims(i, j - 2, k, QV)) *
           0.5_rt * dxinv[1];
    dwdy = (3 * prims(i, j, k, QW) - 4 * prims(i, j - 1, k, QW) +
            prims(i, j - 2, k, QW)) *
           0.5_rt * dxinv[1];
    dTdy = (3 * prims(i, j, k, QT) - 4 * prims(i, j - 1, k, QT) +
            prims(i, j - 2, k, QT)) *
           0.5_rt * dxinv[1];
  }
...
AMREX_GPU_DEVICE AMREX_FORCE_INLINE void ViscousFluxGP(
  ...
  if (markers(i + 1, j, k, 0)) {  // the point to the right is solid then
    ...
    dudx = (1.5_rt * prims(i, j, k, QU) - 2.0_rt * prims(i - 1, j, k, QU) +
            0.5_rt * prims(i - 2, j, k, QU)) *
           dxinv[0];
    dvdx = (1.5_rt * prims(i, j, k, QV) - 2.0_rt * prims(i - 1, j, k, QV) +
            0.5_rt * prims(i - 2, j, k, QV)) *
           dxinv[0];
    dwdx = (1.5_rt * prims(i, j, k, QW) - 2.0_rt * prims(i - 1, j, k, QW) +
            0.5_rt * prims(i - 2, j, k, QW)) *
           dxinv[0];
    dTdx = (1.5_rt * prims(i, j, k, QT) - 2.0_rt * prims(i - 1, j, k, QT) +
            0.5_rt * prims(i - 2, j, k, QT)) *
           dxinv[0];
  ...
  if (markers(i, j + 1, k, 0)) {  // the point to the top is solid then
    ...
    dudy = (1.5_rt * prims(i, j, k, QU) - 2.0_rt * prims(i, j - 1, k, QU) +
            0.5_rt * prims(i, j - 2, k, QU)) *
           dxinv[1];
    dvdy = (1.5_rt * prims(i, j, k, QV) - 2.0_rt * prims(i, j - 1, k, QV) +
            0.5_rt * prims(i, j - 2, k, QV)) *
           dxinv[1];
    dwdy = (1.5_rt * prims(i, j, k, QW) - 2.0_rt * prims(i, j - 1, k, QW) +
            0.5_rt * prims(i, j - 2, k, QW)) *
           dxinv[1];
    dTdy = (1.5_rt * prims(i, j, k, QT) - 2.0_rt * prims(i, j - 1, k, QT) +
            0.5_rt * prims(i, j - 2, k, QT)) *
           dxinv[1];
  ...
  if (markers(i, j, k - 1, 0)) {  // the point into the screen is solid then
    ...
    dudz = (-1.5_rt * prims(i, j, k, QU) + 2.0_rt * prims(i, j, k + 1, QU) -
            0.5_rt * prims(i, j, k + 2, QU)) *
           dxinv[2];
    dvdz = (-1.5_rt * prims(i, j, k, QV) + 2.0_rt * prims(i, j, k + 1, QV) -
            0.5_rt * prims(i, j, k + 2, QV)) *
           dxinv[2];
    dwdz = (-1.5_rt * prims(i, j, k, QW) + 2.0_rt * prims(i, j, k + 1, QW) -
            0.5_rt * prims(i, j, k + 2, QW)) *
           dxinv[2];
    dTdz = (-1.5_rt * prims(i, j, k, QT) + 2.0_rt * prims(i, j, k + 1, QT) -
            0.5_rt * prims(i, j, k + 2, QT)) *
           dxinv[2];
  }
