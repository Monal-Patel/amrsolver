...
AMREX_GPU_DEVICE AMREX_FORCE_INLINE
void viscfluxes (int i, int j, int k, auto const& prims, auto const& fx, auto const& fy, auto const& fz, GpuArray<Real,AMREX_SPACEDIM> const& dxinv, PROB::ProbClosures const& closures) noexcept
{
...
AMREX_GPU_DEVICE AMREX_FORCE_INLINE
void viscfluxes_wall (int i, int j, int k, int loc, auto const& prims, auto const& fx, auto const& fy, auto const& fz, GpuArray<Real,AMREX_SPACEDIM> const& dxinv, PROB::ProbClosures const& closures) noexcept
{
    // 2nd order accurate central difference of primitive vars /////////////////
    // x direction
    Real ux   =  prims(i,j,k,QU);
