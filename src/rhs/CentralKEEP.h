...
class keep_euler_t
{
  ...
  keep_euler_t () {
    ...
    coeffs(0,0) = 1.0; coeffs(0,1) = 0.0; coeffs(0,2) = 0.0;
    coeffs(1,0) = 4.0/3; coeffs(1,1) = -2.0/12; coeffs(1,2) = 0.0;
    coeffs(2,0) = 6.0/4; coeffs(2,1) = -6.0/20; coeffs(2,2) = 2.0/60;
  }
  ...
  void inline eflux(const Geometry& geom, const MFIter& mfi,const Array4<Real>& prims, const Array4<Real>& cons, const Array4<Real>& flx, const Array4<Real>& rhs, const closures& cls) {
    ...
    Array1D<Real,0,2> order_coeffs{coeffs(halfsten-1,0),coeffs(halfsten-1,1),coeffs(halfsten-1,2)}; // get coefficients array for current scheme
    ...
    for (int dir=0; dir<AMREX_SPACEDIM; dir++){
      ...
      {
        rhs(i,j,k,n) += dxinv[dir]*(flx(i,j,k,n) - flx(i+1,j,k,n));
      });
    }
  ...
  void flux_dir(int i, int j, int k, int dir, const Array1D<Real,0,2>& coefs, const Array4<Real>& prims, const Array4<Real>& flx, const closures& cls) const {

      for (int n=0; n<NCONS; n++) {flx(i,j,k,n)=0.0;};
      ...
      for (int l=1; l<=halfsten; l++) {
        for (int m=0; m<=l-1; m++) {
          ...
          flx(i,j,k,UMX)  += coefs(l-1)*fghCubic(rho1,rho2,uu1,uu2,ux1,ux2);
          flx(i,j,k,UMY)  += coefs(l-1)*(fghCubic(rho1,rho2,uu1,uu2,uy1,uy2) + fDiv(p1,p2));
          flx(i,j,k,UMZ)  += coefs(l-1)*fghCubic(rho1,rho2,uu1,uu2,uz1,uz2);
          flx(i,j,k,UET)  += coefs(l-1)*(massflx*ke + fghCubic(rho1,rho2,uu1,uu2,ie1,ie2)+ fgDiv(p1,p2,uu1,uu2));
        }
      }
    }
  ...
  void inline eflux_ibm() {
    amrex::Print() << "IBM eflux" << std::endl;
  }
  ...
  //Quadratic split
  AMREX_GPU_DEVICE AMREX_FORCE_INLINE 
  Real fgQuad(Real f,Real fl, Real g, Real gl) const {
    return 0.25_rt*(f + fl)*(g + gl);}

  //Cubic split
  AMREX_GPU_DEVICE AMREX_FORCE_INLINE 
  Real fghCubic(Real f,Real fl, Real g, Real gl,Real h, Real hl) const {
    return 0.1250_rt*(f + fl)*(g + gl)*(h + hl);}
};


// Immersed boundaries ///////////////////////////////////////////////////
// TODO: the following if results in a compilation error with USE_IBM as all branches of constexpr if are evaluated at compile time and IBMultiFab is not defined without USE_IBM. This is a problem as we want to use the same code for both IBM and non-IBM cases.
// if constexpr (PROB::do_ibm==1) {
//   IBM::IBMultiFab& mfab = *IBM::ibMFa[level]; // this is ugly
//   // field array
//   auto& ibfab = mfab.get(mfi);
//   auto const& conFabArr  = consmf.array(mfi); // this is a const because.array() returns a const but we can still modify conFab as consmf input argument is not const
