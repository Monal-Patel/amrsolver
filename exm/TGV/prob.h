...
void prob_initdata (int i, int j, int k, Array4<Real> const& state, GeometryData const& geomdata, ProbClosures const& cls, const ProbParm& pparm) {
  ...
  u[0] = v0 * sin(pparm.omega_x * x / pparm.L) *
         cos(pparm.omega_y * y / pparm.L) *
         cos(pparm.omega_z * z / pparm.L);
  u[1] = -v0 * cos(pparm.omega_x * x / pparm.L) *
         sin(pparm.omega_y * y / pparm.L) *
         cos(pparm.omega_z * z / pparm.L);
  if (pparm.convecting) {
    u[0] += v0;
    u[1] += v0;
...
///////////////////////////////AMR//////////////////////////////////////////////
AMREX_GPU_DEVICE AMREX_FORCE_INLINE 
void user_tagging(int i, int j, int k, int nt_level, auto& tagfab, const auto &sdatafab, const auto& geomdata, const ProbParm& pparm , int level) {}
////////////////////////////////////////////////////////////////////////////////
